(function () {
    ...
    /**
     * ПЛАГІН: UA COLLECTIONS
     * РОЗРОБНИК: Lampa Users
     * ВЕРСІЯ: 1.0
     * ОПИС: Додає пункт меню з добірками українського контенту.
     */

    /* --- Маніфест плагіна --- 
       Метадані для системи Lampa. */
    var pluginManifest = {
        name: 'Українські колекції',
        version: '1.0',
        description: 'Фільми, серіали та мультфільми українського виробництва',
        author: 'Lampa Users',
		    docs: 'Private Server',
    ...
    Lampa.Lang.add({
        uaSelect_menu_title: { en: "Ukrainian Collections", uk: "Українські колекції" },
        uaSelect_ua_films: { en: "Ukrainian Films", uk: "Українські фільми" },
        uaSelect_ua_series: { en: "Ukrainian Series", uk: "Українські серіали" },
        uaSelect_ua_cartoons: { en: "Ukrainian Cartoons", uk: "Українські мультфільми" },
        uaSelect_ua_cartoon_series: { en: "Ukrainian Cartoon Series", uk: "Українські мультсеріали" },
        uaSelect_sorting: { en: "Sorting", uk: "Сортування" },
        uaSelect_vote_count_desc: { en: "Most Votes", uk: "Багато голосів" },
        uaSelect_vote_average_desc: { en: "High Rating", uk: "Високий рейтинг" },
        uaSelect_first_air_date_desc: { en: "New Releases", uk: "Новинки" },
        uaSelect_popularity_desc: { en: "Popular", uk: "Популярні" },
        uaSelect_revenue_desc: { en: "Audience Interest", uk: "Інтерес глядачів" }
    });
    ...
    function applySortParams(sort, options) {
        ...
        if (sort.id === 'first_air_date.desc') {
            var end = new Date(now); end.setDate(now.getDate() - 10);
            var start = new Date(now); start.setFullYear(start.getFullYear() - 1);
            params += '&first_air_date.gte=' + start.toISOString().split('T')[0] + '&first_air_date.lte=' + end.toISOString().split('T')[0];
        }

        if (sort.id === 'release_date.desc') {
            var end = new Date(now); end.setDate(now.getDate() - 40);
            var start = new Date(now); start.setFullYear(start.getFullYear() - 1);
            params += '&release_date.gte=' + start.toISOString().split('T')[0] + '&release_date.lte=' + end.toISOString().split('T')[0];
        }
    ...
    function showUASelectMenu() {
        ...
        Lampa.Select.show({
            ...
            items: items,
            onSelect: function (item) { showSortList(item); },
    ...
    /* --- Реєстрація в бічному меню --- */
    function initPlugin() {
        // ТВОЯ ОРИГІНАЛЬНА SVG-ІКОНКА (Карта України)
        ...
        menuItem.on('hover:enter', showUASelectMenu);
