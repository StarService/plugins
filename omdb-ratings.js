(function() {
    ...
    /* --- Маніфест плагіна для відображення в налаштуваннях --- */
    var pluginManifest = {
        name: 'OMDB Ratings',
        version: '1.0',
        description: 'Розширені рейтинги (IMDB, RT, Metacritic) з розрахунком середнього балу',
        author: 'Lampa Users',
		docs: 'Private Server',
    ...
    Lampa.Lang.add({
        rating_omdb_avg: { // Виправлено на rating
            ru: 'ИТОГ',
            en: 'TOTAL',
    ...
    /* --- Конфігурація Кешу (STAR) --- */
    var CACHE_TIME = 3 * 24 * 60 * 60 * 1000; // 3 дні
    var OMDB_CACHE = 'star_rating_omdb';      // Замінено на star
    var ID_MAPPING_CACHE = 'star_rating_id_mapping'; // Замінено на star
    ...
    function fetchAdditionalRatings(card) {
        ...
        if (imdbElement.length > 0 && !!imdbElement.find('> div').eq(0).text().trim()) {
            updateUI(); // Викликаємо оновлення UI навіть якщо IMDB є
            return;
        }
    ...
    function insertRatings(rtRating, mcRating, oscars) {
        ...
        if (mcRating && !isNaN(mcRating) && !$('.rate--mc', rateLine).length) {
            var mcElement = $(
                ...
                '<div>' + mcRating.toFixed(1) + '</div>' +
                '<div class="source--name"></div>' +
                '</div>'
            );
            mcElement.find('.source--name').html(Lampa.Lang.translate('source_mc'));
            var after = $('.rate--rt', rateLine).length ? $('.rate--rt', rateLine) : lastRate;
            if (after.length) mcElement.insertAfter(after);
        ...
        if (oscars && !isNaN(oscars) && oscars > 0 && !$('.rate--oscars', rateLine).length) {
            ...
            oscarsElement.find('.source--name').html(Lampa.Lang.translate("star_omdb_oscars"));
    ...
    function calculateAverageRating() {
        ...
        if (ratings.imdb > 0) { weightedSum += ratings.imdb * WEIGHTS.imdb; totalWeight += WEIGHTS.imdb; ratingsCount++; }
        if (ratings.tmdb > 0) { weightedSum += ratings.tmdb * WEIGHTS.tmdb; totalWeight += WEIGHTS.tmdb; ratingsCount++; }
        if (ratings.mc > 0) { weightedSum += ratings.mc * WEIGHTS.mc; totalWeight += WEIGHTS.mc; ratingsCount++; }
        if (ratings.rt > 0) { weightedSum += ratings.rt * WEIGHTS.rt; totalWeight += WEIGHTS.rt; ratingsCount++; }
        ...
        if (ratingsCount > 1 && totalWeight > 0) {
            ...
            var avgElement = $(
                ...
                '<div class="source--name">' + Lampa.Lang.translate("rating_omdb_avg") + '</div>' + // Виправлено на rating
                '</div>'
            );
            rateLine.prepend(avgElement);
        }
    ...
    function startPlugin() {
        if (window.star_combined_ratings_loaded) return;
        window.star_combined_ratings_loaded = true;
